...
pub unsafe fn compile_and_run(stream: &[usize; crate::compiler::parser::MAX_STREAM_WORDS],
                               len: usize,
                               yields_value: bool)
    -> Result<Option<u32>, ()>
{
    ...
    while ip < len {
        ...
        if word == word_of(primitives::lit_prim) {
            ...
            let addr_context =
                next == word_of(primitives::load_reg_prim)
            ...
            em.emit_add(regs::ACC, regs::ACC, regs::SCRATCH).map_err(|_| ())?;
            acc_loaded = true;
        } else if word == word_of(primitives::sub_prim) {
            em.emit_sub(regs::ACC, regs::ACC, regs::SCRATCH).map_err(|_| ())?;
            acc_loaded = true;
        } else if word == word_of(primitives::mul_prim) {
            em.emit_mul(regs::ACC, regs::ACC, regs::SCRATCH).map_err(|_| ())?;
            acc_loaded = true;
        } else if word == word_of(primitives::div_prim) {
            em.emit_div(regs::ACC, regs::ACC, regs::SCRATCH).map_err(|_| ())?;
            acc_loaded = true;
            ...
            em.emit_store_u32(regs::SCRATCH, regs::ACC).map_err(|_| ())?;
        } else {
...
fn is_compilable(stream: &[usize; crate::compiler::parser::MAX_STREAM_WORDS], len: usize) -> bool {
    ...
    while ip < len {
        ...
        if w == add_w || w == sub_w || w == mul_w || w == div_w
            || w == load_w || w == write_w
        {
