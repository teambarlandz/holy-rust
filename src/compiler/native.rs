...
mod regs {
    pub const ACC: u8 = 10; // a0 (x10)
    pub const SCRATCH: u8 = 11; // a1 (x11)
}
...
pub unsafe fn compile_and_run(
    ...
    // EXEC_BUFFER now lives in DTIM (@0x80001000) per memory-riscv.x; QEMU
    // softmmu executes RW/RWX RAM without enforcing PT_LOAD X flags, and on
    // real FE310 the boot ROM leaves PMP permissive. Native path is live on
    // both architectures.
